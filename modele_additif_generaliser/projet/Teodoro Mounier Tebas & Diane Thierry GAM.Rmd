...
knitr::include_graphics(here::here("./bernstein_basis.png"))
```
...
```{r echo=FALSE, fig.align='center', fig.height=7, fig.width=15, out.width="120%"}
# On plote les valeurs prédites et les valeurs observées pour voir la qualité du modèle simple
...
recap_modele_ID1 <- data.frame(GAM.1 = c(4, 3, 0.3448, "/", round(gam1_ID1$aic,2), round(MAPE_gam1_ID1,4), "X"), 
                               ...
                               GAM.4 = c(3, 3, 0.862, 7.53, round(gam4_ID1$aic,2), round(MAPE_gam4_ID1,4),"✓"))

row.names(recap_modele_ID1) <- c("Nombre d'arguments", "Nombre d'arguments significatifs", "R2", "GCV", "AIC", "MAPE", "Validation")

kbl(recap_modele_ID1, align = "c") %>% kable_classic(full_width = F,html_font = "Cambria") %>% column_spec(5, color = 'green')
```
...
recap_modele_ID2 <- data.frame(GAM.1 = c("TAP_VERSION", "s(Week,k=1)", "s(Day,k=1)", "s(Hour)", "3/4", 0.164, "/", round(gam1_ID2$aic,2), round(MAPE_gam1_ID2,4), "X"), 
                               GAM.2 = c("s(Day,k=7)", "s(Hour,k=48)", "", "", "2/2", 0.3576, 321.82, round(gam2_ID2$aic,2), round(MAPE_gam2_ID2,4),"✓"), 
                               GAM.3.1 = c("te(Day,Hour,k=c(7,48))", "", "", "", "1/1", 0.3779, 317.5, round(gam3.1_ID2$aic,2), round(MAPE_gam3.1_ID2,4),"✓"), 
                               GAM.3.2 = c("te(Week,Hour,k=c(7,48))", "", "", "", "1/1", 0.3742, 317.82, round(gam3.2_ID2$aic,2), round(MAPE_gam3.2_ID2,4),"✓"),
                               GAM.3.3 = c("te(Day,Hour,k=c(7,48)", "", "", "", "1/1", 0.3722, 319.8, round(gam3.3_ID2$aic,2), round(MAPE_gam3.3_ID2,4),"✓"),
                               GAM.3.4 = c("te(Day,Hour,k=c(7,48))", "te(Week,Hour,k=c(7,48))", "", "", "2/2", 0.4204, 302.04, round(gam3.4_ID2$aic,2), round(MAPE_gam3.4_ID2,4),"✓"),
                               GAM.3.5 = c("te(Day,Hour,k=c(7,48))", "te(Week,Hour,k=c(7,48))", "te(Week,Day,k=c(7,7))", "", "3/3",  0.4313, 296.87, round(gam3.5_ID2$aic,2), round(MAPE_gam3.5_ID2,4),"✓"))
...
recap_modele_ID3 <- data.frame(GAM.1 = c("TAP_VERSION", "TAP_EXIT_STATUS", "s(Week)", "s(Day,k=1)", "s(Hour)", "4/5", 0.639, "-", round(gam1_ID3$aic,2), round(MAPE_gam1_ID3,4), "X"), 
                               GAM.2 = c("TAP_EXIT_STATUS", "s(Week,k=10)", "s(Day,k=7)", "s(Hour,k=24)", "", "4/4", 0.681, 62305.42, round(gam2_ID3$aic,2), round(MAPE_gam2_ID3,4),"✓"), 
                               GAM.3.1 = c("TAP_EXIT_STATUS", "te(Week,Day,k=c(10,7))", "te(Hour,Day,k=c(24,7))", "te(Hour,Week,k=c(24,10))", "", "4/4", 0.885, 26049, round(gam3_ID3$aic,2), round(MAPE_gam3_ID3,4),"✓"))
